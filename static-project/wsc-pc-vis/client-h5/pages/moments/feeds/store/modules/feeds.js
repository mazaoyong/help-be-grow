...
const feeds = {
  ...
  actions: {
    async fetchFeedList({ state, commit }, payload = {}) {
      try {
        const res = await ajax({
          loading: false,
          method: 'GET',
          url: '/v4/vis/h5/edu/moments/feeds/getFeedList.json',
          data: {
            pageRequest: {
    ...
    async handleEditMoreForDetail({ state, commit }, payload = {}) {
    ...
    async handleEditMore({ state, commit }, payload = {}) {
      try {
        ...
        feedList.forEach(feed => {
          if (postId && feed.postId === postId) {
            feed.showEditMore = !feed.showEditMore;
          } else {
            feed.showEditMore = false;
          }
        });
