...
import formDecorator from './form-decorator';
  ...
  return (
    <div className={klass} style={style}>
      <div className={labelKlass}>
        {label || ' '}
        {!!labelDesc && <div className={labelDescKlass}>{labelDesc}</div>}
      </div>
      {children}
    </div>
  );
...
export const FormPreview = ({ children, style }) => {
  return (
    <div className={`${prefix}plain-text`} style={style}>
      {children}
    </div>
  );
};
