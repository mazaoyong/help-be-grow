...
class Select extends React.Component {
   ...
   */
  uniformData(props) {
    const { data, children, optionValue, optionText, resetOption, resetText } = props;
   ...
   *
   * @param {Array<Object>} data - uniformed data
   * @param {Object} props - props of Select
   * @memberof Select
   */
  ...
  locateSelected(state, coord, item, i) {
    ...
    if (isArray(value)) {
      ...
      if (
        valueIndex > -1 &&
        !state.sItems.find(selected => selected && selected.value === item.value)
      ) {
  ...
  optionChangedHandler = (ev, selectedItem) => {
    ...
    const { onEmptySelected, optionValue, optionText, onChange } = this.props;
      ...
      >
        ...
        <Content>
          <Popup
            ...
            autoWidth={autoWidth}
            adjustPosition={this.popover && this.popover.adjustPosition.bind(this.popover)}
          />
        </Content>
      </Popover>
    );
