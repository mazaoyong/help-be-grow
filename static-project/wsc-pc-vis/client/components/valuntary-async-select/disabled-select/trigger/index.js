...
export default class SelectClickTrigger extends Popover.Trigger.Click {
  ...
  render() {
    const { onClick, trigger, onTriggerRefChange, onReset, text, ...rest } = this.props;
    const Node = decideTrigger(trigger);
    // 如果是禁用的option，仅仅展示纯文本部分，而不展示disable.msg部分
    const pureText = Array.isArray(text) ? text[0] : text;
    return <Node {...rest} text={pureText} ref={onTriggerRefChange} onReset={onReset} onClick={this.clickHandler} />;
  }
}
