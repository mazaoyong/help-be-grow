...
import { Tag, Button, Notify, BlockLoading, Icon } from 'zent';
...
const SystemTagGroup = function({ tags }) {
        ...
        <Pop trigger="hover" content={
          ...
          根据默认规则自动打标
            <a
              ...
              className="cursor-link tags-dialog__group__title-pop-link"
              href="https://help.youzan.com/displaylist/detail_13_13-1-54980"
            >查看规则</a>
          </span>
        }>
          <Icon type="help-circle" style={{ color: 'rgb(132,133,135)' }} />
        </Pop>
      </p>
      <ul className="tags-dialog__group__content">
        {tags.map(({ tagId, name }) => (
          <li
            key={tagId}
            className="tags-dialog__group__item"
          >
            <Tag
              theme="grey"
            >
    ...
    <div className="tags-dialog__wrapper">
      <BlockLoading loading={loading}>
        <article className="tags-dialog">
          <SystemTagGroup tags={systemTags} />
          {options.map(option => (
            <TagGroup
              key={option.groupId}
              data={option}
          ...
          {options.length === 0 && (
            <p className="tags-dialog__empty">暂无标签 <a href={`${_global.url.v4}/vis/edu/page/clue/tags`} target="_blank" rel="noopener noreferrer" >去创建</a></p>
          )}
        </article>
      </BlockLoading>
      <DialogFooter>
        <div>
          ...
          <SamWrapper name="编辑标签">
            <span className="tags-dialog__link-group">
              ...
              <a href="/v4/vis/edu/page/clue/tags" target="_blank" rel="noopener noreferrer">添加标签</a>
            </span>
          </SamWrapper>
        </div>
        <div>
