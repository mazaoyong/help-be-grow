...
import { Dialog, LinkGroup, ListPopupEditor, EasyList } from '@youzan/ebiz-components';
...
const HAS_EDIT_PERMISSION = checkAccess('编辑');
class SourceList extends Component {
  ...
  isOwner = HAS_EDIT_PERMISSION && window._global.kdtId === this.props.kdtId;
  ...
  // 删除来源
  handleSourceDelete = (sourceIds, _name) => () => {
  ...
  handleDelete = (sourceId) => {
    return batchDeleteSource({ sourceIds: [sourceId] }).then(() => {
      Notify.success('删除来源成功');
      this.getSourceList();
    })
      .catch(err => {
        Notify.error(err);
      ...
      <div className="clue-source-list">
        ...
        <Grid
          ...
          batchRender={() => (
            this.isOwner ? (
              <div>
                <label style={{ marginRight: '16px' }}>当页全选</label>
    ...
    },
    {
      ...
      bodyRender: row => {
        ...
        return <LinkGroup data={[
          ...
          >编辑</SamLink>,
          <span key={`delete-${sourceId}`} className="source-delete-action">
            <GridPop
              ...
              content="确认删除该来源吗？"
              onConfirm={() => {
                return this.handleDelete(sourceId);
              }}
              onCancel={() => {}}
            />
          </span>,
