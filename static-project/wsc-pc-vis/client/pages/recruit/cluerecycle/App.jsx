...
class ClueRecycle extends PureComponent {
  ...
  fetchData = ({ pageConditions, filterConditions }) => {
    ...
    if (+pageNumber > 100) {
      return Promise.reject('分页数不能大于100页');
    }

    if (+pageNumber * pageSize > 3000) {
      return Promise.reject('查询总数不能超过3000条');
    }

    const recycleBinQuery = omit(filterConditions, ['recordDateRange']);
    const deleteTime = filterConditions.recordDateRange;

    if (Array.isArray(deleteTime)) {
      const [selectedTime] = deleteTime;
      const [timeRange] = selectedTime || [];
      const [startTime, endTime] = timeRange || [];

      recycleBinQuery.recordDateRange = {};
      if (startTime) {
        recycleBinQuery.recordDateRange.startTime = format(Number(startTime), FORMAT);
      }
      if (endTime) {
        recycleBinQuery.recordDateRange.endTime = format(Number(endTime), FORMAT);
      }
    }
