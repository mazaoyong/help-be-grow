...
class Select extends (PureComponent || Component) {
   ...
   * @returns {Array<Object>} uniformedData - 格式化后对象数组
   ...
   *
   * @param {Array<Object>} data - uniformedData
   * @param {Object} props - props of Select
   * @memberof Select
   */
  ...
  triggerBlurHandler = data => {
    let { onSearchChange } = this.props;
    if (onSearchChange) {
      onSearchChange(data.keyword);
      this.setState({
        open: false,
        keyword: null,
      });
  ...
  optionChangedHandler = (ev, selectedItem) => {
    // 由于选择 option 时会同时触发 trigger 的 blur 事件
    // 50毫秒内同时触发两个事件时应阻止触发 blur
    this.optionChangedTime = +new Date();
    const result = {};
    ev = ev || {
      preventDefault: noop,
    ...
    // 当点击规格项时，如果接口创建失败 此时文本依然显示给商家造成困惑
    const hasValue = selectedItem.value > 0;
    if (tags && hasValue) {
  ...
  render() {
    ...
    return (
      <Popover
        ...
        visible={open}
        className={`zent-select__popover ${prefixCls} ${popupClassName}`}
      ...
      >
        <PopoverClickTrigger>
          <Trigger
            ...
            onTriggerBlur={this.triggerBlurHandler}
            maxLength={maxLength}
          />
        </PopoverClickTrigger>
        <Popover.Content>
