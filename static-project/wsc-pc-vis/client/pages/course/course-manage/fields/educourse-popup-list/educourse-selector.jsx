...
export default function EducourseSelector(props) {
  const isShopSelected = useRef(null);
  const [openDialog, SchoolDialog] = SchoolChooser({
    componentConfig: {
      ...
      columns: arrayColumnWrapper([
        ...
        },
        {
          ...
          bodyRender: ({ applicableCampusType, applicableCampusList }) => {
            return <>{applicableCampusType ? <span>全部校区</span>
              : isShopSelected.current ? <span>—</span> : <Pop trigger="hover" block content={applicableCampusList.map(item => item.shopName).join('；')}><span><span style={{ color: '#38f' }}>{applicableCampusList ? applicableCampusList.length : 0}</span>个</span></Pop>}</>
            ;
          },
      ...
      emptyLabel: (
        <>
          暂无课程，你可以
          <a href={`${_global.url.v4}/vis/edu/page/educourse#/add`} target="_blank" rel="noopener noreferrer">新建课程</a>
        </>
      )
    },
    dialogConfig: {
...
function Foot(props) {
  return <div style={{ textAlign: 'right' }}><span><Button type='primary' onClick={props.submit}>确定</Button></span></div>;
}
...
function Filter(props) {
  ...
  return <div className="educourse-choose-header">
    <div>
      <Button
        type="primary"
      ...
      <Button onClick={() => fetchData({ filterConditions: filterInfo, pageConditions: pageInfo })}>
        刷新
      </Button>
    </div>
