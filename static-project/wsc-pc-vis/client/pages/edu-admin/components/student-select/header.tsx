...
import { Button, Input, Checkbox } from 'zent';
...
function getHeaderButton(child: IHeaderChild, props: IDialogProps) {
  const { text, childProps = {} } = child;
  const { fetch } = props;
  return <Button onClick={fetch} {...childProps }>{text}</Button>;
}
  ...
  return (
    <Select
      ...
      onChange={(_d, selected) => {
        change({ [name]: selected.value }).then(fetch);
      }}
      {...restProps}
    />
  );
