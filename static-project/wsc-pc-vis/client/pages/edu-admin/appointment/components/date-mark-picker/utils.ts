import { startOfDay, startOfMonth, endOfDay, format as formatBase, parse } from 'date-fns';
...
export function setTime(date: Date, time: Date | string = TIME_BEGIN): Date {
  ...
  const dateTimeArr: any = [date.getFullYear(), date.getMonth(), date.getDate(), ...timeArr];
