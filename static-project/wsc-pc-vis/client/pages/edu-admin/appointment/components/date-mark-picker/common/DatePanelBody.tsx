...
import { isWithinRange, startOfToday } from 'date-fns';
...
export default class DatePanelBody extends React.PureComponent<any> {
  isSelected(val) {
  ...
  hasMark = val => {
    ...
    if (!mark || mark.length === 0) return false;
    const year = '' + val.getFullYear();
    const month = '' + (val.getMonth() + 1);
    const date = '' + val.getDate();
    return mark.includes(`${year}-${month.padStart(2, '0')}-${date.padStart(2, '0')}`);
  };

  beforeToday = val => {
    return val.getTime() < startOfToday().getTime();
  }
    ...
    for (let rowIndex = 0; rowIndex < ROW; rowIndex++) {
      ...
      for (let colIndex = 0; colIndex < COL; colIndex++) {
        ...
        const className = classNames({
          ...
          'date-mark': this.hasMark(val),
          'date-mark-gray': this.beforeToday(val),
