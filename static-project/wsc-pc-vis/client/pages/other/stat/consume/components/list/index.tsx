...
import { Button, Notify, GridTextAlign } from 'zent';
...
import { findConsumeClassDetailPage, exportConsumeClassDetail } from '../../api';
...
function ConsumeList() {
  ...
  const columns = useMemo(() => {
    ...
    return (
      [
        ...
        {
          title: '课时消耗数',
          name: 'consumedLessonCnt',
          width: '20%',
          needSort: true,
          bodyRender: ({ consumedLessonCnt }) => {
            return consumedLessonCnt / 100;
          },
        },
        {
          title: '课时消耗金额（元）',
          name: 'consumedLessonAmt',
          width: '20%',
          textAlign: 'right' as GridTextAlign,
          needSort: true,
          bodyRender: ({ consumedLessonAmt }) => {
            return formatMoney({ num: consumedLessonAmt });
          },
        },
      ]
    );
  ...
  const exportList = useCallback((state) => {
    getNewFetchParams(state).then(params => {
      exportConsumeClassDetail(params)
        .then(data => {
          if (data) {
  ...
  const openExportRecord = () => {
    window.open(`${_global.url.v4}/vis/stat/export/consume#/export/14`);
  };
  ...
  const fetchList = useCallback<IListProps['onSubmit']>(state => {
    return new Promise((resolve, reject) => {
      getNewFetchParams(state).then(params => {
        ...
        findConsumeClassDetailPage(params)
          .then(res => {
  ...
  const clsPrefix = 'consume-list-container';

  return (
    <div className={clsPrefix}>
      <div className={`${clsPrefix}__header`}>消课明细</div>
