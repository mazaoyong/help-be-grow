...
import { Input, Checkbox } from 'zent';
...
const prefix = 'decorate-title-text-editor';
...
export default class TitleEditor extends DesignEditor {
  ...
  renderWx = () => {
    ...
    if (wxLinkType === '1') {
      help = getWeappNotSuppertedLinkMessage(wxLink) || getH5NotSuppertedLinkMessage(wxLink);
    }
      ...
      <div className={`${prefix}__wx`}>
        ...
        <ControlGroup
          ...
          help={
            help && (
              <>
                <HelpIcon help={help} type="error-circle" /> 该链接在小程序内点击不跳转
              </>
            )
          }
        >
          {wxLinkType === '0' ? (
            <LinkTag url={getUrl('/setting/weixin', 'www', globalConfig.url)}>
              设置快速关注链接
            </LinkTag>
          ) : (
            <ChooseMenu
  ...
  render() {
    ...
    const noticeMsg =
      globalConfig.is_weapp_setting === IS_WEAPP_SETTING
        ? `（小程序路径需要小程序 v${WEAPP_VERSION_MAP.title} 版本及以上）`
        : '';
