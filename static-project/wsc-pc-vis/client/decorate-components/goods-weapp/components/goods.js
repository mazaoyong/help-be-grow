...
import chooseWSCGoods from '@youzan/react-components/es/components/choose-dialog/dialogs/goods';
import chooseGoods from '@youzan/react-components/es/components/choose-dialog/dialogs/weapp-goods';
import chooseGoodsTag from '@youzan/react-components/es/components/choose-dialog/dialogs/goods-tag';
import { Input, Radio } from 'zent';
import { ControlGroup, EditorCard, GoodsImage, LinkTag, HelpDesc } from '../../common/';
import EditorCardAdd from '../../common/editor-card/add';
...
export default class GoodsHeader extends Component {
  ...
  addGoodImage = () => {
    ...
    if (globalConfig.is_weapp_setting === IS_WEAPP_SETTING) {
      chooseGoods({
        config: globalConfig,
        multiple: true,
        onChoose(list) {
  ...
  render() {
    ...
    return (
      <>
        <ControlGroup label="添加商品" required style={{ padding: '24px 16px 10px' }}>
