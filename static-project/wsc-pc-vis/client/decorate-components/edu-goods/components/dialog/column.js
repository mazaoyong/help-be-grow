...
export default function(config, goodList) {
  return baseConfig({
    title: '选择专栏',
    url: getUrl('/vis/course/column/getList.json', 'v4', config.url),
    columns: [
      {
        title: '商品',
        width: '70%',
      ...
      },
      {
        ...
        // needSort: true,
        bodyRender(data) {
          return format(data.publishAt, 'YYYY-MM-DD HH:mm:ss');
        },
      },
    ],
    actions: [
      {
        ...
        label: '新建',
        onClick() {
          window.open(`${_global.url.v4}/vis/course/column/list`);
        },
      },
      {
        type: 'button',
