...
import Filter from './room-type-filter';
import useList from './use-list';
import style from './index.m.scss';

const dataCache = {};
const roomTypeListLink = '/v4/industry/hotel/room';
const roomTypeCreateLink = '/v4/industry/hotel/room-detail';
const columns = [
  {
    title: '房型',
    name: 'name',
  ...
  },
  {
    ...
    bodyRender(data) {
      return (
        <div className={style.cell}>
          <ClampLines lines={2} popWidth={200} text={data.hotelName} />
        </div>
      );
    },
  },
...
const hotelListDialog = props => {
  ...
  const { listData, page, setPage, total, loading, filter, setFilter } = useList({
    history: false,
    getData(params) {
      const { page, hotelId, name: search } = params;
      return ajax('/v4/shop/design/hotel/getRoomTypeByPage.json', {
        data: {
          hotelId,
          search,
          page,
  ...
  const selectedContent = useMemo(() => `已选 ${select.length} 个房型`, [select]);
  const refresh = useCallback(() => {
    setPage(page);
  }, [page, setPage]);
  const onConfirm = useCallback(() => {
    if (select.length > 50) {
      Notify.error('所选房型已经超过最大上限50个');
      return;
    }
  ...
  return (
    <div className={style.dialog}>
      <div className={style.header}>
        <div>
          <Button target="_blank" href={roomTypeCreateLink}>
            新建
          </Button>
          <Button target="_blank" href={roomTypeListLink}>
            房型管理
          </Button>
