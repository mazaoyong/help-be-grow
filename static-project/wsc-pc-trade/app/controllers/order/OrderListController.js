...
class OrderListController extends OrderBaseController {
    ...
    try {
      ...
      const [tagConfig, hoverConfig] = await Promise.all([
        apolloClient.getConfig({
          appId,
          namespace,
          key: 'exception.order.tag',
        }),
        apolloClient.getConfig({
          appId,
          namespace,
          key: 'exception.order.hover',
        }),
      ]);

      this.ctx.setGlobal('riskOrderConfigs', {
        tag: tagConfig,
        hover: hoverConfig,
      });
    } catch (e) {
  ...
  async getIndexHtml(ctx) {
    await Promise.all([
      ...
      this.initABCEPayShopWhiteList(ctx),
      this.initWechatDeliveryWhiteList(ctx),
    ]);

    await ctx.render('order/list.html');
  }
  ...
  async initABCEPayShopWhiteList(ctx) {
    const { kdtId } = ctx;
    const abcEPay = await this.grayRelease('abc_epay', kdtId);
    ctx.setGlobal({ abcEPay });
  }
}
