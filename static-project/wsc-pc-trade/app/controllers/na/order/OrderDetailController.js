...
class OrderDetailController extends BaseController {
  ...
  async getIndexHtml(ctx) {
    ...
    await ctx.render('na/order/detail.html');
  }

  async getShareOfRefundFee(ctx) {
    const { kdtId } = ctx;
    const { orderNo, orderItemId, refundFee = 0 } = ctx.query;
  ...
  // 退课
  async refund(ctx) {
    const refundCommand = ctx.request.body;
  ...
  async activeRefundBySeller(ctx) {
    ...
    const result = await new SellerRefundService(ctx).activeRefundBySeller({
      kdtId,
      operatorId,
      source,
      orderNo,
