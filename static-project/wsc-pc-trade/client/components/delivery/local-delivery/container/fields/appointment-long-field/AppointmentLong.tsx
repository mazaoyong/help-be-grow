...
import { IAppointmentLongValue } from 'definitions/local-delivery';

const RadioGroup = Radio.Group;

export interface IProps {
  value: IAppointmentLongValue;
  onChange: (value: IAppointmentLongValue) => void;
}

const prefix = 'custom-apointment-long';

/**
 * 最长预约字段
 */
class AppointmentLong extends PureComponent<ZENT_FIELD_COMP<IProps>> {
  getDaysOptions() {
  ...
  handleTypeChange = evt => {
    ...
    value.type = evt.target.value as IAppointmentLongValue['type'];
  ...
  handleValChange = (
    ...
    type: IAppointmentLongValue['type'],
  ) => {
    const value = this.cloneValue();
...
// @ts-ignore
export default Form.getControlGroup(AppointmentLong);
