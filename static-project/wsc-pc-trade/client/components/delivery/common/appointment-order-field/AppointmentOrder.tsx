...
import InventoryGuideDialogAnchor from '../inventory-guide-dialog-anchor';
...
/** 下此订单的店铺, 所属的发货模式 */
export enum ISupplyMode {
  /** 供货模式, 常量命名的意思是指 供货链 有实体仓库 */
  STOCK_ENTITY = 0,
  /** 铺货模式, 没有对应的 供货仓, 所以是 VIRTUAL */
  VIRTUAL = 1,
}

// 是否为铺货模式
const isVirtualSupplyMode = window._global.shopSupplyMode === ISupplyMode.VIRTUAL;
...
class AppointmentOrder extends PureComponent<ZENT_FIELD_COMP<IProps>, IState> {
  constructor(props: ZENT_FIELD_COMP<IProps>) {
    ...
    const { hour = 12, minute = 0 } = cutOffRuleModel;
    this.state = {
      cutOffTime: `${padLeft(hour)}:${padLeft(minute)}`,
      err1: false,
      err2: false,
    };
  }
  ...
  handleTypeChange = evt => {
    const data = cloneDeep(this.props.value);
    const value = evt.target.value;

    data.ruleType = value as IAppointmentOrderValue['ruleType'];
  ...
  _renderTimeSelect(
    ...
    const valueOptions = this.getValueOptions(type);
      ...
      <>
        {type === 'minute' ? (
          <NumberInput
            ...
            max={1440}
            width={88}
            className={`${error ? 'show-error' : 'no-error'} mr10`}
        ...
        <Select
          className={`${error ? 'show-error' : 'no-error'} ml0`}
          data={typeOptions}
          optionValue="value"
          optionText="text"
          value={type}
  ...
  _renderTimePicker(isDisabled = false) {
    ...
    return (
      <TimePicker
        className="zent-picker-demo no-error ml0"
        value={cutOffTime}
    ...
    return (
      <div className={prefix}>
        <RadioGroup className={`${prefix}__type`} value={ruleType} onChange={this.handleTypeChange}>
          ...
          {/* 根据仓库类型配置 */}
          {/* 只在零售单店显示该选项 */}
          {isRetailSingleStore && syncPlanResult && !isVirtualSupplyMode && (
            <Radio value={2}>
                ...
                <>
                  ...
                  <div className="row">
                    ...
                    {this._renderTimeSelect(
                      ...
                      planAheadMinType,
                      value =>
                        this.handleFieldsValueChange(value, 'inventoryRuleModel.planAheadMin'),
                      type =>
                        this.handleFieldsTypeChange(
                          type,
