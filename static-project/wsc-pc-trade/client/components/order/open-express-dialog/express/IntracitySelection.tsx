...
class IntracitySelection extends React.Component<IProps, IState> {
   ...
   * 刷新智选价格
   */
  updateAlphaFee(weight: number) {
    const { alphaExpressEnable, orderNo } = this.props;
    const params = {
      orderNo,
      alphaExpressEnable,
      distWeight: weight,
  ...
  renderContent() {
    const { channels, alphaExpressEnable, appointedDeliveryTime, isShowAutoBar } = this.props; // 是否启用第三方同城配送服务
    const autoCallFormatTime = formatDate(appointedDeliveryTime, 'YYYY-MM-DD HH:mm:ss');
