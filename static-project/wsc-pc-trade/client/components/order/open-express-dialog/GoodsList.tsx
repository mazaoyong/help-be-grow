...
import { Table, Alert, ITableColumn } from 'zent';
...
class GoodsList extends React.Component<IProps> {
  ...
  // 有退款时候的提示文案
  renderAlert() {
    const { callSource, model } = this.props;
    const { hasRefund, isExchange } = model;
    let content = '';
    ...
    if (hasRefund) {
      ...
      if (callSource === 2) {
        ...
        content = '订单中包含退款中的商品，对其进行发货将撤销退款申请。';
      }
    }

    if (isExchange) {
      content = '换货商品重新发货不会引起库存变化';
    }
