...
class Intracity extends React.Component<IProps, IState> {
  ...
  renderCallCourier(onSubmit) {
    ...
    const { fee, totalFee, isCloudTag, feeStatus } = this.state.localDelivery;
    // totalFee单位是元，balance单位是分，将totalFee*100转为分再与balace比较
    const balanceLow = toCent(totalFee as string) > balance;
    // 云服务商，feeStatus不等于1表示账户余额不足，不可呼叫，非云服务商，按照原有逻辑
    const callDisabled = isCloudTag ? feeStatus !== 1 : !fee || balanceLow;
    return (
      <div className="call-area">
        <Button loading={submitting} disabled={callDisabled} onClick={onSubmit}>
          呼叫配送员
        </Button>
        {/* 云服务商不需要显示店铺余额不足的情况，由feeStatus决定且在其他地方展示账户余额不足 */}
        {balanceLow && !isCloudTag && (
          <div>
            店铺余额不足，
            <a
