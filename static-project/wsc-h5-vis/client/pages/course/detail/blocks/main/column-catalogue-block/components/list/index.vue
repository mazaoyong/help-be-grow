  ...
  <div>
    ...
    >
      ...
      >
        <!-- 已学完。。。 -->
        <div>
          <span v-for="(text, index) in getDescList(props.item)" :key="index" class="desc-item">
            {{ text }}
          </span>
        </div>
      </goods-card-lite>
...
export default {
  ...
  methods: {
    ...
    fetchPrev() {
      ...
      this.request({ alias: this.goodsData.alias, page: this.lastPage - 1, sortType: this.sortType })
        .then(res => {
          this.prevLoading = false;
          this.$commit('lastPage', this.lastPage - 1);
          this.$refs.list.processList(list => [].concat(res.list, list));
        })
        .catch(() => {
          this.prevLoading = false;
          this.prevError = true;
    ...
    request({ alias, page, sortType }) {
      return getContentAndLive(alias, page, sortType).then(res => {
        this.$dispatch('setTotal', res.total);
        return {
          list: res.content && res.content.map(item => ({ ...item, pageNumber: page })),
    ...
    getDescList(item) {
      ...
      if (item.mediaType === MEDIA_TYPE.VIDEO && item.videoContentDTO && item.videoContentDTO.videoDuration) {
      ...
      if (item.mediaType !== MEDIA_TYPE.LIVE && this.goodsData.isOwnAsset) {
        ...
        if (progress.percent) {
          list.push(progress.percent === 100 ? '已学完' : `${progress.percent || 0}%`);
        }
      }
      return list;
    },
