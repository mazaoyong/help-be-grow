...
const AlternateDomain = {
  'shop46402991': 'shop46402991-01',
};

/* 临时处理被封域名 */
const checkAlternate = (url) => {
  for (let shopUrl in AlternateDomain) {
    if (url.indexOf(shopUrl) > -1) {
      return url.replace(shopUrl, AlternateDomain[shopUrl]);
    }
  }
  return url;
};
...
const formatLinkParams = params => {
  ...
  query.kdt_id = kdtId;
  params.url = Args.add(checkAlternate(params.url), query);
  return params;
};
