...
function getNavigator(appName, jsonApi) {
  ...
  return getTotalFiles(clientPageDir).filter(clientPageFilePath => {
    ...
    if (
      fileStr.indexOf(keyword[0]) !== -1 &&
      fileStr.indexOf(keyword[1]) !== -1
    ) {
      return true
    }
    return false
...
function apiAssemble(appName, api) {
  // 将router里面的api组装成为对象
  let result = []
  if (!Array.isArray(api[0]) && !Array.isArray(api[2])) {
    // 不考虑请求方法或者对应的controller是多个的情况
    let controllerSuffixPath = `/app/controllers/${api[2].replace(
      /\./g,
      '/'
    )}.js`
      ...
      controllerMethod.forEach(controllerFunc => {
        astObj.getServicePath(controllerFunc).forEach(service => {
          ...
          result.push({
            ...
            jsonApi: jsonApiItem,
            navigator: ['wsc-pc-vis', 'wsc-h5-vis'].includes(appName)
              ? getNavigator(appName, jsonApiItem.replace('/v4/vis', '')).map(
                  item => item.match(/\/client\/pages\/.+/)[0]
                )
              : [],
  ...
  try {
    projectInfo.forEach(item => {
      ...
      const routersPath = path.join(
        __dirname,
        `./static-project/${appName}/app/routers`
      )
      ...
      const pb = new ProgressBar(
        `${chalk.blue(
          appName.padEnd(15, ' ')
        )} 解析进度: :percent [:bar] :current/:total`,
        {
          total: apiArr.length,
          width: 50,
      ...
      apiArr.forEach(apiItem => {
        ...
        data = [
          ...
          ...apiAssemble(
            appName,
            apiItem.length > 4 ? apiItem.slice(1) : apiItem
          ),
        ]
        pb.tick()
      })
...
const handleVueFile = file => {
  ...
  const _file = file.slice(
    start + ((vueCheckReg && vueCheckReg.length) || 0),
    end
  )
  ...
  traverse(astFile, {
    enter(path) {
      if (path.isImportDeclaration()) {
        if (
          path.node.source.value.includes(EBIZ_COMPONENTS) ||
...
const astLocalComponentsAnalysis = (astFile, filePath) => {
  const componentList = new Set()

  traverse(astFile, {
    enter(enterPath) {
      if (enterPath.isImportDeclaration()) {
        const pathValue = enterPath.node.source.value || ''
        if (pathValue.includes('components')) {
          const componentsPathReg = /components\/([^\/]+)/
          if (/^(@\/)?(components)/.test(pathValue)) {
            const str = pathValue.match(componentsPathReg)[1]
            componentList.add(str)
          } else {
            if (pathValue.includes('../')) {
              const absolutePath = path.join(filePath, '../', pathValue)
              if (
                absolutePath.includes('client/components') ||
                absolutePath.includes('client-h5/components')
              ) {
                const str = absolutePath.match(componentsPathReg)[1]
                componentList.add(str)
              }
            }
          }
...
const componentMain = projectInfoList => {
  projectInfoList.forEach(projectInfo => {
    const pathname = path.resolve(
      `app/static-project/${projectInfo.name}/client`
    )
    ...
    ;[EBIZ_COMPONENTS, VIS_COMPONENTS, RC_COMPONENTS, VANT_COMPONENTS].forEach(
      componentLibName => {
    ...
    fs.writeFileSync(
      path.resolve(
        __dirname,
        `./output/TEST-COMPONENT-LIST-${projectInfo.name}.json`
      ),
      JSON.stringify(fileComponentsJSON, null, 2),
      {
        encoding: 'utf-8',
      }
    )
...
const localComponentsMain = projectInfoList => {
  projectInfoList.forEach(projectInfo => {
    const pathname = path.resolve(
      `app/static-project/${projectInfo.name}/client`
    )
    const componentLibName = 'localComponents'
    console.log(chalk.blue('组件解析 - start：', pathname))
    ...
    pathnameArr.map(_pathname =>
      rd.eachFileFilterSync(
        ...
        filename => {
          ...
          if (/(from\s(\'|\")(@\/|\.\.\/.*?)?(components))/.test(file)) {
            // console.log('filename ->>>>>', filename);
            const astFile = file2babelAst(file, filename)
            const componentList = astLocalComponentsAnalysis(astFile, filename)
            fileComponentsJSON[componentLibName][filename] = componentList
          }
        }
      )
    )
    if (!fs.existsSync(path.resolve(__dirname, './localComponentsOutput/'))) {
      fs.mkdirSync(path.resolve(__dirname, './localComponentsOutput/'))
    }
    fs.writeFileSync(
      path.resolve(
        __dirname,
        `./localComponentsOutput/TEST-COMPONENT-LIST-${projectInfo.name}.json`
      ),
      JSON.stringify(fileComponentsJSON, null, 2),
      {
        encoding: 'utf-8',
